...
APARTMENTS = [
    ...
    {"name": "Wesselenyi Sarga", "airbnb_ical": "https://www.airbnb.hu/calendar/ical/1669541670319961133.ics?t=762f114e031d42f7887a527b57c9fb54", "booking_ical": "https://ical.booking.com/v1/export?t=f42d26be-80b6-4dec-93bb-b5eff4bfe37e", "google_cal_id": "2403d05221582141fffd60db8adf0d87b2782ce155da77bbc98d9df1e9458c16@group.calendar.google.com", "airbnb_cal_id": "541d33a0ebf9950b308298831fa2eb60cc1dc34a42a8306eacb1c5f81e7de86f@group.calendar.google.com"},
    {"name": "Wesselenyi Kek", "airbnb_ical": "https://www.airbnb.hu/calendar/ical/1669508090758767119.ics?t=5daac341dd9943ca8e5b3809efb2908e", "booking_ical": "https://ical.booking.com/v1/export?t=e75fbe46-65ee-45a7-ac18-a0d7d55e79c3", "google_cal_id": "5c67a41037e447c7e94a3c60d8065e7d9449bebeacd587f570d826768bb9caf9@group.calendar.google.com", "airbnb_cal_id": "77f895ee7d61f1b9f7a0436f3ad848832010f93170ac230f2d8ee207dfc7538f@group.calendar.google.com"},
    {"name": "Dohany28 Onyx", "airbnb_ical": "https://www.airbnb.hu/calendar/ical/1640651701407031274.ics?t=d171325e11204af88c73a717c962c4da", "booking_ical": "https://ical.booking.com/v1/export?t=3ab247b7-ba53-4639-9ea9-d8425e1ef385", "google_cal_id": "99dad39c0d3afd68c65de4592d2e944ed5eec03b40aa6eaaf482b587858341f9@group.calendar.google.com", "airbnb_cal_id": "1aba99966f1c02998f29aba06258a20007416f47a53ee6a1916b404356e92eef@group.calendar.google.com"},
    {"name": "Dohany47", "airbnb_ical": "https://www.airbnb.hu/calendar/ical/54403858.ics?t=0e46f9f4092747bfa79656cc5fb2e5f6", "booking_ical": "https://ical.booking.com/v1/export?t=2a7d6f55-71db-4758-8015-d1874ca9e20a", "google_cal_id": "482f32b95114d01b9f25c8879a9d63656b61b44cb028caddb7b1f0bd32c7cdce@group.calendar.google.com", "airbnb_cal_id": "b647ba4bd202e35db98c5237e0e77d0381690369634c14fb6b9e424ac4dd0136@group.calendar.google.com"},
]
...
def sync_to_google_calendar(cal_id, all_bookings, apt_name):
    ...
    try:
        ...
        # Get existing [SW] events from Google Calendar
        existing = {}  # key: "start_end" -> event_id
        page_token = None
        while True:
            result = svc.events().list(calendarId=cal_id, singleEvents=True, pageToken=page_token).execute()
            for e in result.get("items", []):
                if "[SW]" not in e.get("summary", ""):
                    continue
        ...
        # Current bookings as set of keys
        current_keys = {f"{b['start'].strftime('%Y-%m-%d')}_{b['end'].strftime('%Y-%m-%d')}" for b in all_bookings}

        # Add only NEW bookings (never delete first)
        for b in all_bookings:
            key = f"{b['start'].strftime('%Y-%m-%d')}_{b['end'].strftime('%Y-%m-%d')}"
            if key not in existing:
                ...
                # Touch existing event to bust Airbnb cache
                svc.events().patch(calendarId=cal_id, eventId=existing[key], body={
                    "description": f"Last synced: {ts}"
                }).execute()

        # Remove ONLY cancelled bookings (ones no longer in the feed)
